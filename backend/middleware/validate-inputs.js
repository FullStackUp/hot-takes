...
//Validation des données d'entrée lors du signup et/ou login d'un utilisateur
...
const sauceSchema = Joi.object({
    ...
    manufacturer: Joi.string().trim().min(1).max(20).required(),
    description: Joi.string().trim().min(1).max(400).required(),
    mainPepper: Joi.string().trim().min(1).max(40).required(),
...
//validation de l'id pour les sauces
...
//Validation pour les likes et dislikes d'une sauce
